...
func TestReElection2A(t *testing.T) {
	...
	DPrintf(dTest, "disconnect %d, a new one should be elected.", leader1)
	...
	DPrintf(dTest, "connect %d, shouldn't disturb the new leader.", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no new leader should
	// be elected.
	DPrintf(dTest, "disconnect %d and %d", leader2, (leader2+1)%servers)
	...
	DPrintf(dTest, "connect %d, it should elect a leader.", (leader2+1)%servers)
	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	DPrintf(dTest, "connect %d, shouldn't disturb the new leader.", leader2)
...
func TestManyElections2A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		DPrintf(dTest, "RECONNECTING %d, %d, %d", i1, i2, i3)
		cfg.connect(i1)
		cfg.connect(i2)
		cfg.connect(i3)
	}
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	DPrintf(dTest, "disconnected %d, a new one should be elected.", leader1)
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	DPrintf(dTest, "disconnected %d, a new one should be elected.", leader2)

	// old leader connected again
	cfg.connect(leader1)
	DPrintf(dTest, "%d back online, should not mess up logs.", leader1)

	cfg.one(104, 2, true)

	// all together now
	cfg.connect(leader2)
	DPrintf(dTest, "%d back online, should not mess up logs.", leader2)
